...
import type { IMetricsStorage } from "./metrics-storage.interface.js";
...
export class RedisStorageStrategy implements IMetricsStorage {
  ...
  async getHistory(machineId: string): Promise<MachineMetric[]> {
    ...
    const results = await this.redis.zrevrangebyscore(machineKey, "+inf", "-inf");
  ...
  async getMetricsInTimeRange(machineId: string, since: Date): Promise<MachineMetric[]> {
    ...
    const results = await this.redis.zrangebyscore(machineKey, minScore, maxScore);

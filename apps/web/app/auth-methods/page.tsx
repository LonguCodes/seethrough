...
export default function AuthMethodsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      await api.delete(`auth-methods/${id}`);
      fetchData();
    } catch {
  ...
  const handleToggle = async (m: AuthMethod) => {
    try {
      await api.patch(`auth-methods/${m.id}`, { json: { enabled: !m.enabled } });
      fetchData();
    } catch {
  ...
  const renderSettingsForm = () => {
    ...
    const setSettings = (partial: Record<string, any>) =>
      setForm({ ...form, settings: { ...form.settings, ...partial, type: form.type } });
          ...
          <div className="space-y-4 p-4 rounded-xl bg-white/[0.02] border border-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input
                  type="number"
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-[var(--accent)]"
                  value={s.minPasswordLength || 8}
                  onChange={(e) => setSettings({ minPasswordLength: +e.target.value })}
                />
              </div>
              <div className="flex items-center justify-between p-3 rounded-xl bg-white/[0.03]">
                <div>
                  <p className="text-sm text-slate-200">Require Complexity</p>
                  <p className="text-xs text-slate-500">Mix of upper/lowercase, numbers, symbols</p>
                </div>
                <button
                  type="button"
                  onClick={() => setSettings({ requireComplexity: !s.requireComplexity })}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${s.requireComplexity ? "bg-[var(--accent)]" : "bg-white/10"}`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${s.requireComplexity ? "translate-x-6" : "translate-x-1"}`}
                  />
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-[var(--accent)]"
                  value={s.issuerUrl || ""}
                  onChange={(e) => setSettings({ issuerUrl: e.target.value })}
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-[var(--accent)]"
                  value={s.clientId || ""}
                  onChange={(e) => setSettings({ clientId: e.target.value })}
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) => setSettings({ clientSecret: e.target.value })}
                placeholder={editingId ? "••••••••" : "secret"}
                required={!editingId}
              />
            </div>
          </div>
      ...
      case "saml":
        return (
          <div className="space-y-4 p-4 rounded-xl bg-white/[0.02] border border-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-sm text-slate-400">Entry Point</label>
                <input
                  required
                  type="url"
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-[var(--accent)]"
                  value={s.entryPoint || ""}
                  onChange={(e) => setSettings({ entryPoint: e.target.value })}
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-[var(--accent)]"
                  value={s.issuer || ""}
                  onChange={(e) => setSettings({ issuer: e.target.value })}
            ...
            <div className="space-y-2">
              ...
              <textarea
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-[var(--accent)] font-mono text-xs"
                value={s.cert || ""}
                onChange={(e) => setSettings({ cert: e.target.value })}
    ...
    return (
      <div className="p-8 max-w-7xl mx-auto min-h-screen">
        <header className="flex items-center gap-4 mb-12">
          ...
          <h1 className="text-4xl text-gradient">Auth Methods</h1>
        </header>
      ...
      {showForm && (
        <section className="mb-12 glass p-8 rounded-3xl animate-in fade-in slide-in-from-top-4 duration-300">
          <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
            ...
            {editingId ? "Edit Auth Method" : "New Auth Method"}
          </h2>
                ...
                >
                  {Object.entries(METHOD_TYPES).map(([k, v]) => (
                    <option
                      key={k}
                      value={k}
                      disabled={k === "password" && hasPasswordMethod && !editingId}
                    >
                      {v}
                    </option>
                  ))}
            ...
            <div className="space-y-4 p-4 rounded-xl bg-white/[0.02] border border-white/5">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-white/[0.03]">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Create accounts on first SSO login</p>
                </div>
                <button
                  type="button"
                  onClick={() => setForm({ ...form, autoCreateUsers: !form.autoCreateUsers })}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${form.autoCreateUsers ? "bg-[var(--accent)]" : "bg-white/10"}`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${form.autoCreateUsers ? "translate-x-6" : "translate-x-1"}`}
                  />
                </button>
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div className="glass p-16 text-center rounded-3xl flex flex-col items-center gap-6">
            ...
            <p className="text-slate-400 max-w-md">
              No auth methods configured. Add one to enable authentication.
            </p>
          </div>
        ) : (
            ...
            <AnimatePresence mode="popLayout" initial={false}>
              {sortedMethods.map((m, index) => (
                ...
                >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <button
                          ...
                          disabled={m.enabled && isOnlyEnabled}
                          title={
                            m.enabled && isOnlyEnabled
                              ? "Cannot disable the only enabled method"
                              : m.enabled
                                ? "Disable"
                                : "Enable"
                          }
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            Auto-create: {m.autoCreateUsers ? `Yes (${m.defaultRole})` : "No"}
                            {m.mfaConfig ? ` · MFA: ${m.mfaConfig.name}` : ""}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`p-2 rounded-xl transition-colors ${isOnlyMethod ? "bg-white/[0.03] text-slate-600 cursor-not-allowed" : "bg-[var(--danger)]/10 text-[var(--danger)] hover:bg-[var(--danger)]/30"}`}
                          title={isOnlyMethod ? "Cannot delete the only method" : "Delete"}
                        >
                          <Trash2 size={18} />
                        </button>
                      </div>
                  ...
                  <div
                    data-expanded={expandedId === m.id}
                    className={
                      "grid grid-rows-[0fr] data-[expanded=true]:grid-rows-[1fr] duration-300 transition-all"
                    }
                  >
                    <div className="px-6 pb-6 border-t border-white/5 pt-4 space-y-4 overflow-hidden">
                      <div>
                        <span className="text-xs text-slate-500">Settings:</span>
                        <pre className="mt-1 p-3 bg-black/30 rounded-lg text-xs text-slate-300 font-mono overflow-x-auto">
                          ...
                          {mfaConfigs.length > 0 && (
                            <select
                              ...
                              onChange={async (e) => {
                                ...
                                try {
                                  await api.patch(`auth-methods/${m.id}`, {
                                    json: { mfaConfigId: val || null },
                                  });
                                  fetchData();
                                } catch {

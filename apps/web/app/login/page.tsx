...
function LoginForm() {
  ...
  const {
    ...
    queryFn: async () => {
      const data = await api.get("auth/configurations").json<AuthConfiguration[]>();
      localStorage.setItem("cached-auth-configs", JSON.stringify(data));
      return data;
    },
    placeholderData: () => {
      ...
      if (cached) {
        try {
          return JSON.parse(cached) as AuthConfiguration[];
        } catch {
          /* ignore */
        }
      }
  ...
  const loginMutation = useMutation({
    mutationFn: async (body: Record<string, unknown>) => {
      const data = await api.post("auth/login", { json: body }).json<Record<string, unknown>>();
      return data;
    },
    onSuccess: (data) => {
      if (data.mfaRequired && data.mfaChallenge) {
        setMfaRequired(true);
        setMfaChallenge(data.mfaChallenge as MfaChallenge);
        setMfaType((data.mfaChallenge as MfaChallenge).mfaMethodType || "totp");
        if ((data.mfaChallenge as MfaChallenge).mfaMethodType !== "passkey") {
          setTimeout(() => mfaInputRef.current?.focus(), 100);
        }
        return;
      }
      setTokens(data as unknown as AuthTokens);
      refresh();
      router.push("/");
      router.refresh();
    },
    ...
    onError: (err: unknown) => {
      ...
      if (err instanceof HTTPError) {
        void err.response
          .json()
          .then((data: Record<string, unknown>) =>
            setMfaError((data.message as string) || "Invalid verification code"),
          );
      } else {
  ...
  const handleSsoStart = async (config: AuthConfiguration) => {
    ...
    try {
      const data = await api
        .post("auth/login", { json: { configId: config.id } })
  ...
  const handlePasskeyAuth = useCallback(async () => {
    ...
    try {
      ...
      const options = await api
        .post("mfa/passkey/authenticate-options", {
          json: { challengeToken: mfaChallenge?.challengeToken },
        })
        .json<PasskeyAuthOptions>();
      ...
      const assertion = (await navigator.credentials.get({
        publicKey: {
          ...
          allowCredentials: options.allowCredentials?.map((cred) => ({
            ...cred,
            id: Uint8Array.from(
              Buffer.from(cred.id.replace(/-/g, "+").replace(/_/g, "/"), "base64").toString(),
              (c) => c.charCodeAt(0),
            ),
          })),
        },
    ...
    <div className="fixed inset-0 flex items-center justify-center p-6 bg-[#020617] overflow-hidden">
      ...
      <div className="relative w-full max-w-md animate-in fade-in zoom-in duration-700">
        <div className="bg-slate-900/40 backdrop-blur-2xl border border-slate-800 rounded-3xl p-8 shadow-2xl overflow-hidden group">
          ...
          {/* SSO Buttons */}
          {(!configsLoading || (configsLoading && isPlaceholderData && configs.length > 0)) &&
            ssoConfigs.length > 0 && (
          ...
          <div className="mt-8 pt-6 border-t border-slate-800 text-center">
            <p className="text-sm text-slate-500">Contact your administrator if you need access.</p>
          </div>
        </div>
